  ...
  const handleButtons = (name, status) => {
    setButtons((prevButtons) =>
      prevButtons?.map((button) => {
        return button.name === name ? { ...button, status: status } : { ...button }
      })
    );
  };
  ...
  const handleInputs = (name, e) => {
    setFields((prevFields) =>
      prevFields?.map((field) => {
        return field.name === name ? { ...field, input: e.target.value } : { ...field }
      })
    );
  };
  ...
  return (
    <div className="bg-white">
      <div className="max-w-xl mx-auto py-16 px-4 sm:py-8 sm:px-6 lg:max-w-7xl lg:px-8">
        <h2 id="products-heading" className="ml-8 mb-12 text-2xl">
          {" "}
          Settings{" "}
        </h2>
        <span className="block flex-grow">
          <div className="flex justify-between">
            ...
            {buttons[0].status ? (
              ...
              <button
                onClick={() => {
                  handleButtons("firstName", !buttons[0].status);
                }}
              >
                Update
              </button>
          ...
          <div className="flex justify-between">
            ...
            {buttons[1].status ? (
              ...
              <button
                onClick={() => {
                  handleButtons("lastName", !buttons[1].status);
                }}
              >
                Update
              </button>
          ...
          <div className="flex justify-between">
            ...
            {buttons[2].status ? (
              ...
              <button
                onClick={() => {
                  handleButtons("email", !buttons[2].status);
                }}
              >
                Update
              </button>
          ...
          <div className="flex justify-between">
            ...
            {buttons[3].status ? (
              ...
              <button
                onClick={() => {
                  handleButtons("photo", !buttons[3].status);
                }}
              >
                Update
              </button>
