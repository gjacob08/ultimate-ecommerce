...
export default function Inventory() {
  const toggleImportProductModal = useGlobal(
    (state) => state.toggleImportProductModal
  );
  const toggleUpdateProductModal = useGlobal(
    (state) => state.toggleUpdateProductModal
  );
    ...
    const getProductsEffect = () => {
      Axios.get("http://localhost:5000/api/products", {
        ...
        .then((resObject) => {
          ...
          categories = ["All"];
          for (let data of resObject.data)
            if (!categories.includes(data.category))
              categories.push(data.category);
          categories.push(resObject.data.category);
          let filteredProducts;
          selectedSortCategory === "All"
            ? (filteredProducts = resObject.data)
            : (filteredProducts = resObject.data.filter((product) =>
                product.category.includes(selectedSortCategory)
              ));
  ...
  return (
    <div className="bg-white">
      <div className="max-w-xl mx-auto py-16 px-4 sm:py-8 sm:px-6 lg:max-w-7xl lg:px-2">
        <h2 id="products-heading" className="ml-8 text-2xl">
          {" "}
          Inventory{" "}
        </h2>
            ...
            <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
              <div className="flex items-center">
                ...
                <Listbox
                  ...
                  onChange={setSelectedSortCategory}
                >
                  {({ open }) => (
                    <>
